...
const temporaryRepository = () => mkdtempSync(join(tmpdir(), "spiral-cli-test-"));

const readJson = (path) => JSON.parse(readFileSync(path, "utf8"));

describe("composition resolution", () => {
  it("composes GitHub + Standard with the compatibility binding", () => {
    const composition = resolveComposition({ artifact: "github", process: "standard" });
  ...
  it("does not pull Standard packages into GitHub + custom", () => {
    const composition = resolveComposition({ artifact: "github", process: "custom" });
    expect(composition.dependencies["@mydx/spiral-standard"]).toBeUndefined();
    expect(composition.dependencies["@mydx/spiral-standard-github"]).toBeUndefined();
  ...
  it("adds Quality Guard only when requested", () => {
    ...
    expect(resolveComposition({ quality: false }).devDependencies).not.toHaveProperty(
      "@mydx/spiral-quality",
    );
  });
});
...
describe("portable init", () => {
  it("initializes an empty repository", () => {
    ...
    try {
      initRepository({ cwd, artifact: "github", process: "standard", quality: true });
      ...
      expect(packageJson.dependencies).toHaveProperty("@mydx/spiral-standard-github");
      expect(packageJson.devDependencies).toHaveProperty("@mydx/spiral-quality");
      ...
      expect(readFileSync(join(cwd, "src/spiral/execution-channel.mjs"), "utf8")).toContain(
        "TODO: project固有Execution Channel",
      );
    } finally {
  ...
  it("preserves an existing TypeScript repository and unrelated workflow", () => {
    ...
    try {
      ...
      writeFileSync(join(cwd, ".github/workflows/ci.yml"), "name: Existing CI\n");
  ...
  it("is idempotent when run twice with the same options", () => {
    const cwd = temporaryRepository();
    try {
      const first = initRepository({ cwd, artifact: "github", process: "standard", quality: true });
      const packageAfterFirst = readFileSync(join(cwd, "package.json"), "utf8");
      const second = initRepository({ cwd, artifact: "github", process: "standard", quality: true });
      expect(first.changes.length).toBeGreaterThan(0);
      expect(second.alreadySatisfied).toBe(true);
      expect(readFileSync(join(cwd, "package.json"), "utf8")).toBe(packageAfterFirst);
    } finally {
      rmSync(cwd, { recursive: true, force: true });
    }
  });
  ...
  it("aborts before writing when an existing generated file conflicts", () => {
    ...
    try {
      ...
      writeFileSync(join(cwd, "spiral.config.mjs"), "export default { custom: true };\n");
  ...
  it("runs fully specified init without prompting", () => {
    ...
    try {
      ...
      expect(readJson(join(cwd, "package.json")).devDependencies).toHaveProperty(
        "@mydx/spiral-quality",
      );
    } finally {
      rmSync(cwd, { recursive: true, force: true });
